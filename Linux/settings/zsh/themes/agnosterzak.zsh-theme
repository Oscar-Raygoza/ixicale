...
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    prompt_segment black white "%{$fg_bold[white]%(!.%{%F{white}%}.)%}SSH: [$HOST]@%m%{$fg_no_bold[white]%}"
  else
    prompt_segment none white "%{$fg_bold[white]%(!.%{%F{white}%}.)%}hostname: [$HOST]%{$fg_no_bold[white]%}"
  fi
}
...
#«»±˖˗‑‐‒ ━ ✚‐↔←↑↓→↭⇎⇔⋆━◂▸◄►◆☀★☗☊✔✖❮❯⚑⚙
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ $has_diverged == false && $commits_ahead -gt 0 ]]; then
      if [[ $bgclr == 'red' || $bgclr == 'black' ]] then
        to_push=" $fg_bold[white]↑$commits_ahead$fg_bold[$fgclr]"
      else
        to_push=" $fg_bold[black]↑$commits_ahead$fg_bold[$fgclr]"
      fi
    fi
    if [[ $has_diverged == false && $commits_behind -gt 0 ]]; then to_pull=" $fg_bold[black]↓$commits_behind$fg_bold[$fgclr]"; fi
